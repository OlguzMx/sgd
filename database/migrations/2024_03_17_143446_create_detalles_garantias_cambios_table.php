<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('detalles_garantias_cambios', function (Blueprint $table) {
            $table->id();
            $table->foreignId('garantias_cambios_id')->constrained('garantias_cambios')->onDelete('cascade');
            $table->string('marca_danado');
            $table->string('modelo_danado');
            $table->string('num_serie_danado');
            $table->string('marca_reemplazo');
            $table->string('modelo_reemplazo');
            $table->string('num_serie_reemplazo');
            $table->string('num_inventario');
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('detalles_garantias_cambios');
    }
};
