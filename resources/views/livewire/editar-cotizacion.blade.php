    ...
    <form wire:submit="editarCotizacion">
        <div class="gap-y-4 sm:gap-x-6 sm:gap-y-4">
            ...
            <div class="grid grid-cols-2 gap-8">
                ...
                <div>
                    <label for="folio" class="block">Folio</label>
                    <input wire:model="folio" id="folio"
                        class="block w-full rounded-md border-gray-300 shadow-sm
                    focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm mb-2">
                    @error('folio')
                        <div
                    ...
                    <tbody>
                        @foreach ($detalles as $index => $detalle)
                            <tr>
                                ...
                                <td class="border px-4 py-2">
                                    <input id="unidad_{{ $index }}" type="text" class="w-full rounded-md"
                                        wire:model.defer="new_detalles.{{ $index }}.unidad_{{ $index }}"
