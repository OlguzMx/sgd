            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-8 py-2">
                ...
                <div class="flex flex-col mt-10">
                    <div class="py-2 overflow-x-auto">
                        ...
                        <!-- FIN - BOTÓN DE CREAR DOCUMENTO -->

                        <div class="align-middle inline-block min-w-full shadow overflow-hidden sm:rounded-lg border-b border-gray-200">

                            <!-- INICIO - TABLA DE DOCUMENTOS -->

                            <table class="min-w-full border rounded overflow-hidden">
                                <thead class="bg-cyan-500 text-white">
                                    <tr>
                                        <th class="px-6 py-3 text-left font-semibold uppercase">Título</th>
                                        <th class="px-6 py-3 text-left font-semibold uppercase">Cliente</th>
                                        <th class="px-6 py-3 text-left font-semibold uppercase">Tipo de Documento</th>
                                        <th class="px-6 py-3 text-left font-semibold uppercase">Usuario</th>
                                ...
                                <tbody id="listado-proveedores" class="bg-slate-50 hover:bg-slate-100">
                                    ...
                                    <tr class="border-t hover:bg-gray-100" id="documento_{{ $documento->id}}">
                                        <td class="px-6 py-4">{{ $documento->titulo }}</td>
                                        <td class="px-6 py-4">{{ $documento->cliente->name }}</td>
                                        <td class="px-6 py-4">{{ $documento->tipo_documento->name }}</td>
                                        <td class="px-6 py-4">{{ $documento->user->name }}</td>
                                        <td class="px-6 py-4">

                                            <!-- INICIO - BOTONES DE EDITAR Y ELIMINAR DOCUMENTOS -->

                                            <div class="flex items-center gap-4">
                                                <a href="{{ route('documentos.edit', $documento->id)}}" title="Editar el Documento" class="bg-green-500 hover:bg-green-600 text-white p-1 rounded">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-6 h-6">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                                    </svg>
                                                </a>
                                                <a href="#" title="Eliminar al Documento" class="bg-red-500 hover:bg-red-600 text-white p-1 rounded" onclick="deleteDocumento({{ $documento->id }})">
                                            ...
                                            <!-- FIN - BOTONES DE EDITAR Y ELIMINAR DOCUMENTOS -->
